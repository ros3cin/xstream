...
public abstract class AbstractReflectionConverter implements Converter, Caching {
    ...
    protected void doMarshal(final Object source, final HierarchicalStreamWriter writer, final MarshallingContext context) {
        ...
        reflectionProvider.visitSerializableFields(source, new ReflectionProvider.Visitor() {
            ...
            final Set<String> writtenAttributes = new java.util.LinkedHashSet<>();
                ...
                if (converter != null) {
                    ...
                    if (value != null) {
                        if (writtenAttributes.contains(fieldName)) {
                            final ConversionException exception = new ConversionException("Cannot write field as attribute for object, attribute name already in use");
        ...
        new Object() {
            ...
            {
                final Map<String, Set<Mapper.ImplicitCollectionMapping>> hiddenMappers = new org.apache.commons.collections4.map.HashedMap<String, Set<Mapper.ImplicitCollectionMapping>>();
                for (final FieldInfo info : fields) {
                    if (info.value != null) {
                        ...
                        if (mapping != null) {
                            ...
                            for (final Iterator<?> iter = isArray ? new ArrayIterator(info.value) : isCollection ? ((Collection<?>) info.value).iterator() : isEntry ? ((Map<?, ?>) info.value).entrySet().iterator() : ((Map<?, ?>) info.value).values().iterator(); iter.hasNext(); ) {
            ...
            void writeField(final String fieldName, final String aliasName, final Class<?> fieldType, final Class<?> definedIn, final Object newObj) {
                ...
                writer.startNode(aliasName != null ? aliasName : mapper.serializedMember(sourceType, fieldName), actualType);
                if (newObj != null) {
    ...
    public Object doUnmarshal(final Object result, final HierarchicalStreamReader reader, final UnmarshallingContext context) {
        ...
        while (it.hasNext()) {
            ...
            if (field != null && shouldUnmarshalField(field)) {
                ...
                final SingleValueConverter converter = mapper.getConverterFromAttribute(classDefiningField, attrName, type);
                if (converter != null) {
        ...
        while (reader.hasMoreChildren()) {
            ...
            final Mapper.ImplicitCollectionMapping implicitCollectionMapping = mapper.getImplicitCollectionDefForFieldName(fieldDeclaringClass, fieldName);
            final Object value;
        ...
        if (implicitCollectionsForCurrentObject != null) {
            for (final Map.Entry<FieldLocation, Collection<? super Object>> entry : implicitCollectionsForCurrentObject.entrySet()) {
    ...
    protected Object unmarshallField(final UnmarshallingContext context, final Object result, final Class<?> type, final Field field) {
    ...
    private void handleUnknownField(final Class<?> classDefiningField, final String fieldName, final Class<?> resultType, final String originalNodeName) {
    ...
    private void writeValueToImplicitCollection(final Object value, final Map<FieldLocation, Collection<? super Object>> implicitCollections, final Object result, final FieldLocation fieldLocation) {
        Collection<? super Object> collection = implicitCollections.get(fieldLocation);
        if (collection == null) {
            final Field field = reflectionProvider.getFieldOrNull(fieldLocation.definedIn, fieldLocation.fieldName);
            final Class<?> physicalFieldType = field != null ? field.getType() : reflectionProvider.getFieldType(result, fieldLocation.fieldName, null);
            if (physicalFieldType.isArray()) {
                ...
                if (!(Collection.class.isAssignableFrom(fieldType) || Map.class.isAssignableFrom(fieldType))) {
                    final ObjectAccessException oaex = new ObjectAccessException("Field is configured for an implicit Collection or Map, but is of an incompatible type");
                ...
                if (instance instanceof Collection) {
                    ...
                    final Mapper.ImplicitCollectionMapping implicitCollectionMapping = mapper.getImplicitCollectionDefForFieldName(fieldLocation.definedIn, fieldLocation.fieldName);
                    @SuppressWarnings("unchecked")
    ...
    private class MappingList extends AbstractList<Object> {
        ...
        private final Map<Class<?>, Field> fieldCache = new org.apache.commons.collections4.map.HashedMap<>();
        ...
        public boolean add(final Object object) {
            ...
            final ConversionException exception = new ConversionException("Element  is not defined as entry for implicit map");
